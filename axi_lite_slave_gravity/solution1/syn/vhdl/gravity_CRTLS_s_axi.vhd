...
port (
    ...
    distc                 :out  STD_LOGIC_VECTOR(31 downto 0);
...
-- 0x24 : reserved
-- 0x28 : Data signal of distc
--        bit 31~0 - distc[31:0] (Read/Write)
-- 0x2c : reserved
...
architecture behave of gravity_CRTLS_s_axi is
    ...
    constant ADDR_DISTC_DATA_0 : INTEGER := 16#28#;
    constant ADDR_DISTC_CTRL   : INTEGER := 16#2c#;
    ...
    signal int_distc           : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_DISTC_DATA_0 =>
                        rdata_data <= RESIZE(int_distc(31 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    distc                <= STD_LOGIC_VECTOR(int_distc);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_DISTC_DATA_0) then
                    int_distc(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_distc(31 downto 0));
                end if;
            end if;
        end if;
    end process;
